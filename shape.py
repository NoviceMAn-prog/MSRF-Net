...
import tensorflow as tf
import cv2
from skimage.morphology import dilation,erosion,area_closing,flood_fill,closing,convex_hull_image
...
import glob
img_list = glob.glob("../data/train_aug/images/*.jpg")
mask_list = glob.glob("../data/train_aug_m/masks/*.jpg")
val_img_list = glob.glob("../data/val_img/images/*.jpg")
val_mask_list = glob.glob("../data/val_img_m/masks/*.jpg")
images_pca_path=glob.glob("../data/pca_data/images/*.jpg") 
masks_pca_path=glob.glob("../data/pca_data/masks/*.jpg")
images_cj_list = glob.glob("../data/cj_aug/images/*.jpg")
masks_cj_list = glob.glob("../data/cj_aug/masks/*.jpg")

print(len(mask_list))
image_shape = (256,256,3)
train_list = img_list
train_mask_list = mask_list
for i in range(len(images_pca_path)):
    train_list.append(images_pca_path[i])
    train_mask_list.append(masks_pca_path[i])
for i in range(len(images_cj_list)):
    train_list.append(images_cj_list[i])
    train_mask_list.append(masks_cj_list[i])

print("final training set length",len(train_list),len(train_mask_list))
import random

c = list(zip(train_list, train_mask_list))

random.shuffle(c)

X, Y = zip(*c)
#Y = [get_image(sample_file,320,320,gray=True) for sample_file in mask_list]
#Y = np.array(Y).astype(np.float32)
#downscale_factor = 8
for i in range(len(X)):
    im=X[i].split('\\')[-1].split('.')[0]
    ma = Y[i].split('\\')[-1].split('.')[0]
    
X_tot_val = [get_image(sample_file,256, 256) for sample_file in val_img_list]
X_val,edge_x_val = [],[]
...
Y_tot_val = [get_image(sample_file,256, 256,gray=True) for sample_file in val_mask_list]
Y_val,edge_y = [],[]
for i in range(0,100):
...
def DSup1(x, var):
    ...
    d = UpSampling2D(var)(d)
    return d
def sau(input_size=(256,256,3),input_size_2=(256,256,1)):
    n_labels=1
    feature_scale=8
    ...
    att = Input(input_size_2,name='checkdim')
    input_img = Concatenate()([inputs_img,att])
    n11 = Conv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(input_img)
    ...
    #########################
    pred8 = Conv2D(1,kernel_size=(1,1),strides=(1,1),padding='same',activation="sigmoid")(n43)
    pred8 = UpSampling2D(size=(8,8),interpolation='bilinear',name='pred8')(pred8)
...
def shape(input_size=(256,256,3)):
    n_labels=1
    feature_scale=8
    #input_shape= Image.shape
    filters = [64, 128, 256, 512,1024]
    ...
    model = Model(inputs= inputs_img,outputs = edge_out)
    return model
...
def train(epochs, batch_size,output_dir, model_save_dir):
    ...
    max_val_dice= -1
    G = sau()
    #G.load_weights('unet_connected_4rrdb_att_se_aspp.h5')
    G.summary()
    optimizer = get_optimizer()
    G.compile(optimizer = optimizer, loss = {'x':seg_loss,'pred4':seg_loss,'pred2':seg_loss,'pred8':seg_loss},loss_weights={'x':1.,'pred4':1. , 'pred2':1. , 'pred8':1.})
    S = shape()
    optimizer = get_optimizer()
    S.compile(optimizer = optimizer, loss = {'edge_out':'binary_crossentropy'})
    for e in range(1, epochs+1):
        print ('-'*15, 'Epoch %d' % e, '-'*15,batch_size)
        #sp startpoint
        for sp in range(0,batch_count,1):
            if (sp+1)*batch_size>len(X):
                batch_end = len(X)
            else:
                batch_end = (sp+1)*batch_size
            X_batch_list = X[(sp*batch_size):batch_end]
            Y_batch_list = Y[(sp*batch_size):batch_end]
            ...
            #print(Y_batch.shape)
            S.train_on_batch(X_batch,edge_y)

        y_pred = S.predict(X_val,batch_size=5)
        y_pred = (y_pred >=0.5).astype(int)
        S.save('shape.h5')


    y_pred_shape = S.predict(X_val,batch_size=5)
    att_maps_val=[]
    for jh in range (len(X_val)):
        att_map = y_pred_shape[i,:,:,:]
        att_map= dilation(att_map)
        att_map = convex_hull_image(att_map)
        att_maps_val.append(att_map)
    att_maps_val = np.asarray(att_maps_val).astype(np.float32)
    for e in range(1, epochs+1):
        print ('-'*15, 'Epoch %d' % e, '-'*15,batch_size)
        #sp startpoint
        for sp in range(0,batch_count,1):
            ...
            for i in range(0,batch_size):
                X_batch.append(X_tot[i][0])
                edge_x.append(X_tot[i][1])
                att_map = S.predict(X_tot[i][0])
                att_map = dilation(att_map)
                att_map = convex_hull(att_map)
                att_maps.append(att_map)
            X_batch = np.array(X_batch).astype(np.float32)
            edge_x = np.array(edge_x).astype(np.float32)
            att_maps = np.array(att_maps).astype(np.float32) 
            Y_tot = [get_image(sample_file,256, 256,gray=True) for sample_file in Y_batch_list]
            Y_batch,edge_y = [],[]
            for i in range(0,batch_size):
                Y_batch.append(Y_tot[i][0])
            ...
            G.train_on_batch([X_batch,att_maps],[Y_batch,Y_batch,Y_batch,Y_batch])
        y_pred,_,_,_ = G.predict([X_val,att_maps_val],batch_size=5)
        y_pred = (y_pred >=0.5).astype(int)
...
output_dir = './output/'
train(125,16,output_dir,model_save_dir)
test_img_list = glob.glob("../data/test_img/images/*.jpg")
test_mask_list = glob.glob("../data/test_img_m/masks/*.jpg")
G = sau()
G.load_weights('sau.h5')
G.summary()
optimizer = get_optimizer()
